...
本模块仅填充 `pain_points`、`deal_closing_points`、`churn_points`、`high_frequency_points`。其余分析字段（`persona_tags`、`smart_replies`）返回空数组。`risk_assessment` 使用 `unknown` 等级和分值 `0`，`reason` 写明"本模块不输出风险评估"。
...
- 每条 finding 必须包含 `reason` 与 `evidences`。
...
2. 是否每条 finding 都包含 `reason` 和 `evidences`。
3. `evidences` 的 `quote`、`message_index` 是否都来自输入消息。
4. 对缺乏证据的成交节点和流失节点，是否正确返回了空数组而不是猜测。
5. `persona_tags`、`smart_replies` 是否都是空数组。
